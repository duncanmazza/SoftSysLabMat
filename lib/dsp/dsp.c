        ...
        for (j = 0; j < xlen; j++) {
            if (k >= 0) {
                if (k < hlen) { // k must be from 0 to (length of h - 1)
                    yval += h[k] *
                            x[j];  // add values to y[i] for every overlap
                }
            }
            k = k - 1;
        }
...
output: float complex array that should save DTFT of the array, each element is
complex number
...
float complex *DFT(float data[], int dlen) {
    ...
    float complex *output = (float complex *) malloc(
            sizeof(float complex *) * dlen);
    for (n = 0; n < dlen; n++) {
        real = 0;
        imag = 0;
        for (k = 0; k < dlen; k++) {
            real += creal(data[k] * (cos(2 * PI * n * k / dlen) -
                                     I * sin(2 * PI * n * k / dlen)));
            imag -= cimag(data[k] * (cos(2 * PI * n * k / dlen) -
                                     I * sin(2 * PI * n * k / dlen)));
        }
