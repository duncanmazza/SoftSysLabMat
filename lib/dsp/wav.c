...
float **read_wav(char *filename, int *frequency, int *datalen) {
    ...
    printf("(5-8) Overall size: bytes:%u, Kb:%u n", header.overall_size,
    ...
    header.byterate = buffer4[0] |
                      ...
                      (buffer4[2] << 16) |
                      (buffer4[3] << 24);
    printf("(29-32) Byte Rate: %u , Bit Rate:%un", header.byterate,
           header.byterate * 8);
...
//	if (c == 'Y' || c == 'y') {
    ...
    if ((bytes_in_each_channel * header.channels) != size_of_each_sample) {
        printf("Error: %ld x %ud <> %ldn", bytes_in_each_channel,
    ...
    if (size_is_correct) {
        ...
        printf("nn.Valid range for data values : %ld to %ld n", low_limit,
               high_limit);
        for (i = 1; i <= num_samples; i++) {
            ...
            if (read == 1) {
                ...
                for (xchannels = 0; xchannels < header.channels; xchannels++) {
                    ...
                    output[i - 1][xchannels] = data_in_channel / 1.00 / (pow(2,
